...
class ModelArguments:
    ...
    eval_blimp: Optional[bool] = field(
        default=False,
        metadata={
            "help": "whether to run blimp test"
        },
    )
    ...
    eval_ppl: Optional[bool] = field(
        default=False,
        metadata={
            "help": "whether to use ppl for eval"
        },
    )
...
def get_loss(tokenizer, model, sent: str,device) -> float:
        """get the utterance loss; unnormalized """
        # Encode sentences
...
def get_loss_with_retrieval(tokenizer, model, knn_wrapper, sent: str, device) -> float:
    """Compute the sentence loss with retrieval augmentation using kNN."""

    # Step 1: Tokenize the sentence
    inputs = tokenizer(sent, return_tensors="pt").to(device)

    # Step 2: Break into the model for kNN retrieval (setup retrieval system if not done already)
    if not hasattr(model, 'broken_into'):
        knn_wrapper.break_into(model)

    # Step 3: Forward pass to compute logits and capture activations
    with torch.no_grad():
        # Forward pass to get model outputs (logits) and trigger kNN interpolation
        outputs = model(**inputs, labels=inputs.input_ids)

        # The original loss before applying kNN interpolation
        original_loss = outputs.loss.item()

        # Check if post_forward_hook was applied and if kNN retrieval worked
        if hasattr(knn_wrapper.activation_capturer, 'captured') and knn_wrapper.activation_capturer.captured is not None:
            print("kNN retrieval augmentation applied.")

            # Compute the kNN-augmented logits by applying the post_forward_hook
            interpolated_logits = knn_wrapper.post_forward_hook(None, None, outputs.logits)

            # Use the interpolated logits to compute the loss
            loss_fct = torch.nn.CrossEntropyLoss()
            knn_augmented_loss = loss_fct(interpolated_logits.view(-1, model.config.vocab_size), inputs.input_ids.view(-1))

            return knn_augmented_loss.item()
        else:
            print("kNN retrieval augmentation was NOT applied.")
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        raw_datasets = load_dataset(
            data_args.dataset_name, data_args.dataset_config_name, ignore_verifications=True, cache_dir=model_args.cache_dir
        )

        if "validation" not in raw_datasets.keys():

            print('Splitting val set from original dataset')
    ...
    if not (training_args.do_train or data_args.eval_subset == 'train'):
        ...
        print('Do not evaluate on train data')
    ...
    if training_args.do_eval:
        ...
        if model_args.eval_ppl:
            ...
            print('ORIGINAL metric score is:')
            print(metrics)

            if knn_wrapper is not None:
                knn_metrics = knn_wrapper.get_metrics()
                metrics.update(knn_metrics)
                print('Updated metric score is:')
                print(metrics)
            trainer.log_metrics("eval", metrics)
            trainer.save_metrics("eval", metrics)

        elif model_args.eval_blimp:
            print('Running blimp test')
            # print out the results
            data = pd.read_csv(data_args.validation_file)
            print(data)

            # load model and tokenizer
            model = trainer.model.to(training_args.device)
            print('Model has been loaded!')
            tokenizer = trainer.tokenizer
            print('Tokenizer has been loaded!')
            # get loss of the given token
            data['loss'] = data['text'].apply(lambda sent: get_loss(tokenizer, model, sent,training_args.device))

            print(f'Saved the file to {data_args.validation_file}')
            # apply the kNN wrapper;
            if knn_wrapper is not None:
                data['knn_loss'] = data['text'].apply(lambda sent: get_loss_with_retrieval(tokenizer, model, knn_wrapper, sent, training_args.device))
                print('Finished kNN-retrieval')
            data.to_csv(data_args.validation_file)
